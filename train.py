...
from utils.utils import AverageMeter, Logger, latency_profiler, cuda_latency_profiler, get_energy
...
def train_fixed(start_epoch,
    ...
    mac_inputs = torch.randn(1, 3, 32, 32)
    macs, params = profile(fixed_cnn.cpu(), inputs=(mac_inputs, best_arc))
    print("MACs: ", macs)
    print("Params: ", params)
    mem, _ = ArithmeticIntensity(fixed_cnn.cpu(), sample_arc=best_arc, input_dims=(1, 3, 32, 32)).get_metrics()
    print("AI: ", float(macs)/float(mem))
...
def main():
    ...
    for i, (images, labels) in enumerate(data_loaders['train_subset']):
        shared_cnn.cpu()
        images.cpu()
        images = images[0].view(1, 3, 32, 32)
        normalizer = Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        images = F.upsample(images, size=(32, 32), mode='bilinear', align_corners=False)
